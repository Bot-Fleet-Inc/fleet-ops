# 🔍 audit-bot — Operational Configuration

## Issue Polling

audit-bot polls GitHub for assigned work on a fixed interval.

### Primary Query

```
is:issue is:open assignee:botfleet-audit org:Bot-Fleet-Inc
```

- **Interval**: 60 seconds
- **Method**: `gh search issues --assignee=botfleet-audit --state=open --limit=50`
...
```
is:issue is:open label:bot:audit-bot org:Bot-Fleet-Inc
```
...
audit-bot operates under the **DMZ** security tier. The following access rules apply:
...
10. Log startup: `🔍 audit-bot online — session started at <timestamp>`.
11. Begin main loop.
...
2. Post comment on any in-progress issues: `🔍 audit-bot going offline — will resume on restart`.
