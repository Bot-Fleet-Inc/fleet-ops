...
> **Compliance review (read-only)** — Part of the Bot Fleet Inc

## Overview

audit-bot is a persistent autonomous agent responsible for **Compliance review (read-only)** within the AI bot fleet. It runs on a dedicated Proxmox VM and coordinates with other fleet bots via GitHub Issues.
...
| IP | 172.16.10.23 |
| Security Tier | DMZ |
| GitHub User | botfleet-audit |

## Prerequisites

Before deploying audit-bot, ensure:

- [ ] Proxmox VM 413 (``) is provisioned and running.
...
- [ ] Secrets injected into environment:
  - `GITHUB_TOKEN` — GitHub machine user PAT for `botfleet-audit`
...
export BOT_NAME="audit-bot"
export BOT_ROLE="Compliance review (read-only)"
export GITHUB_USER="botfleet-audit"
export VMID="413"
export IP="172.16.10.23"
export HOSTNAME=""
export TIER="DMZ"
export EMOJI="🔍"
...
op read "op://BotFleet/audit-bot/GITHUB_TOKEN" > /dev/null  # verify access
cat > /home/botuser/workspace/.env << 'ENVEOF'
GITHUB_TOKEN=op://BotFleet/audit-bot/GITHUB_TOKEN
ANTHROPIC_API_KEY=op://BotFleet/audit-bot/ANTHROPIC_API_KEY
OP_SERVICE_ACCOUNT_TOKEN=<injected-at-provisioning>
BOT_NAME=audit-bot
BOT_ROLE=Compliance review (read-only)
...
```bash
sudo cp /home/botuser/workspace/ai-bot-fleet-org/bots/audit-bot/{{BOT_SERVICE_NAME}}.service /etc/systemd/system/
...
tail -f /home/botuser/workspace/logs/$(date -u +%Y-%m-%d)-audit-bot.log
```
...
# On Proxmox host
qm stop 413
qm destroy 413 --purge
qm clone 9000 413 --name  --full
qm set 413 --cores <cpu> --memory <ram_mb>
qm resize 413 scsi0 <disk_size>
qm set 413 --net0 virtio,bridge=vmbr1010
qm set 413 --ipconfig0 ip=172.16.10.23/24,gw=172.16.10.1
qm set 413 --onboot 1 --startup order=2
qm set 413 --cicustom "user=local:snippets/-user.yaml"
qm start 413
```
...
op run --env-file=/path/to/botfleet.env -- ssh botuser@172.16.10.23 'cat > ~/.env'
```
...
gh search issues --assignee=botfleet-audit --state=open --label=status:in-progress
```
...
| Process alive | `pgrep -f audit-bot` | PID returned |
| GitHub auth valid | `sudo -u botuser gh auth status` | `Logged in to github.com as botfleet-audit` |
...
| Log | Path |
|-----|------|
| Bot application log | `/home/botuser/workspace/logs/<date>-audit-bot.log` |
| Daily summary | `/home/botuser/workspace/logs/<date>-audit-bot-summary.md` |
...
4. Check IP conflict: ensure no other VM uses 172.16.10.23.
