...
> **Code review and implementation** — Part of the Bot Fleet Inc

## Overview

coding-bot is a persistent autonomous agent responsible for **Code review and implementation** within the AI bot fleet. It runs on a dedicated Proxmox VM and coordinates with other fleet bots via GitHub Issues.
...
| IP | 172.16.10.24 |
| Security Tier | DMZ |
| GitHub User | botfleet-coding |

## Prerequisites

Before deploying coding-bot, ensure:

- [ ] Proxmox VM 414 (``) is provisioned and running.
...
- [ ] Secrets injected into environment:
  - `GITHUB_TOKEN` — GitHub machine user PAT for `botfleet-coding`
...
export BOT_NAME="coding-bot"
export BOT_ROLE="Code review and implementation"
export GITHUB_USER="botfleet-coding"
export VMID="414"
export IP="172.16.10.24"
export HOSTNAME=""
export TIER="DMZ"
export EMOJI="💻"
...
op read "op://BotFleet/coding-bot/GITHUB_TOKEN" > /dev/null  # verify access
cat > /home/botuser/workspace/.env << 'ENVEOF'
GITHUB_TOKEN=op://BotFleet/coding-bot/GITHUB_TOKEN
ANTHROPIC_API_KEY=op://BotFleet/coding-bot/ANTHROPIC_API_KEY
OP_SERVICE_ACCOUNT_TOKEN=<injected-at-provisioning>
BOT_NAME=coding-bot
BOT_ROLE=Code review and implementation
...
```bash
sudo cp /home/botuser/workspace/ai-bot-fleet-org/bots/coding-bot/{{BOT_SERVICE_NAME}}.service /etc/systemd/system/
...
tail -f /home/botuser/workspace/logs/$(date -u +%Y-%m-%d)-coding-bot.log
```
...
# On Proxmox host
qm stop 414
qm destroy 414 --purge
qm clone 9000 414 --name  --full
qm set 414 --cores <cpu> --memory <ram_mb>
qm resize 414 scsi0 <disk_size>
...
qm set 414 --cicustom "user=local:snippets/-user.yaml"
qm start 414
```
...
op run --env-file=/path/to/botfleet.env -- ssh botuser@172.16.10.24 'cat > ~/.env'
```
...
gh search issues --assignee=botfleet-coding --state=open --label=status:in-progress
```
...
| Process alive | `pgrep -f coding-bot` | PID returned |
| GitHub auth valid | `sudo -u botuser gh auth status` | `Logged in to github.com as botfleet-coding` |
...
| Log | Path |
|-----|------|
| Bot application log | `/home/botuser/workspace/logs/<date>-coding-bot.log` |
| Daily summary | `/home/botuser/workspace/logs/<date>-coding-bot-summary.md` |
...
4. Check IP conflict: ensure no other VM uses 172.16.10.24.
