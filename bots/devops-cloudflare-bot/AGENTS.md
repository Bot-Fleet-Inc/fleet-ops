# ☁️ devops-cloudflare-bot — Operational Configuration

## Issue Polling

devops-cloudflare-bot polls GitHub for assigned work on a fixed interval.

### Primary Query

```
is:issue is:open assignee:botfleet-devcloudflare org:Bot-Fleet-Inc
```

- **Interval**: 60 seconds
- **Method**: `gh search issues --assignee=botfleet-devcloudflare --state=open --limit=50`
...
```
is:issue is:open label:bot:devops-cloudflare-bot org:Bot-Fleet-Inc
```
...
devops-cloudflare-bot operates under the **DMZ** security tier. The following access rules apply:
...
10. Log startup: `☁️ devops-cloudflare-bot online — session started at <timestamp>`.
11. Begin main loop.
...
2. Post comment on any in-progress issues: `☁️ devops-cloudflare-bot going offline — will resume on restart`.
